 ...
 * {@code #root.authentication.user.id} 解析当前认证用户的唯一标识并注入方法参数。</p>
 *
 * <p><b>注意：</b>该注解仅在以下两个前提同时满足时 SpEL 才能正确求值：</p>
 * <ul>
 *   <li>认证产生的 {@link Authentication} 为本库的 {@link UserDetailsAuth}
 *   （其 {@code user} 属性承载当前用户）</li>
 *   <li>{@code user} 的实际类型为 {@link RichUserDetails}，
 *   即继承 {@link Identified} 提供 {@code getId()}</li>
 * </ul>
 ...
 * <pre>{@code
 * @GetMapping("/users/{id}")
 * public Object profile(@CurrentUserId String userId) {
 *     return userId;
 * }
 * }</pre>
 *
...
@Documented
@CurrentSecurityContext(expression = "#root.authentication.user.id")
