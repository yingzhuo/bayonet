...
/**
 * 获取当前认证用户的 ID（{@code long} 表示）。
 * <p>组合 Spring Security 的 {@link CurrentSecurityContext}，通过 SpEL
 * {@code #root.authentication.user.getIdAsLong()} 调用
 * {@link RichUserDetails#getIdAsLong()} 解析当前认证用户的 ID 并注入方法参数。</p>
 *
 * <p><b>注意：</b>该注解仅在以下两个前提同时满足时 SpEL 才能正确求值：</p>
 * <ul>
 *   <li>认证产生的 {@link Authentication} 为本库的 {@link UserDetailsAuth}
 *   （其 {@code user} 属性承载当前用户）</li>
 *   <li>{@code user} 的实际类型为 {@link RichUserDetails}（提供 {@code getIdAsLong()}）</li>
 * </ul>
 ...
 * <pre>{@code
 * @GetMapping("/users/{id}")
 * public Long id(@CurrentUserIdAsLong Long id) {
 *     return id;
 * }
 * }</pre>
 *
...
@Documented
@CurrentSecurityContext(expression = "#root.authentication.user.getIdAsLong()")
public @interface CurrentUserIdAsLong {
