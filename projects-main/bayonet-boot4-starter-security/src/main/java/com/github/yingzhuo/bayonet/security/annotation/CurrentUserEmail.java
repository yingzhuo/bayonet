 ...
 * {@code #root.authentication.user.email} 解析当前认证用户的电子邮箱并注入方法参数。</p>
 *
 * <p><b>注意：</b>该注解仅在以下两个前提同时满足时 SpEL 才能正确求值：</p>
 * <ul>
 *   <li>认证产生的 {@link Authentication} 为本库的 {@link UserDetailsAuth}
 *   （其 {@code user} 属性承载当前用户）</li>
 *   <li>{@code user} 的实际类型为 {@link RichUserDetails}（提供 {@code getEmail()}）</li>
 * </ul>
 ...
 * <pre>{@code
 * @GetMapping("/me")
 * public String email(@CurrentUserEmail String email) {
 *     return email;
 * }
 * }</pre>
 *
...
@Documented
@CurrentSecurityContext(expression = "#root.authentication.user.email")
