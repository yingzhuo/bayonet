 ...
 * 支持通过 Hex/Base64 编码密钥、字节数组或 Java 原生 {@link PublicKey}/{@link PrivateKey} 构造实例。
 * SM2 是一种基于椭圆曲线密码学的非对称算法，可视为 JWT 场景下 RSA 等算法的国密替代方案。</p>
 *
 * <p>签名输出为 DER 编码，与标准 {@code SM3withSM2} 完全兼容。</p>
 *
 * <pre>{@code
 * var sm2 = new SM2(publicKey, privateKey);
 * byte[] signature = sm2.sign(content);
 * boolean ok = sm2.verify(content, signature);
 * }</pre>
 *
 * @author 应卓
 * @see SM2Signer
...
public final class SM2 {
    ...
    /**
     * 使用 Hex 或 Base64 编码的密钥对构造实例。
     *
     * @param publicKeyText  Hex 或 Base64 编码的公钥（非空）
     * @param privateKeyText Hex 或 Base64 编码的私钥（非空）
     */
    public SM2(String publicKeyText, String privateKeyText) {
     ...
     * @throws IllegalArgumentException 当密钥对不是合法的 SM2 密钥时
     */
     ...
     * 对内容计算 SM2 签名。
     *
     * @param contentBytes 待签名内容（非 {@code null}）
     * @return 签名（DER 编码，非 {@code null}）
     * @throws RuntimeException 签名失败
     */
    public byte[] sign(byte[] contentBytes) {
        Assert.notNull(contentBytes, "content bytes must not be null");
        try {
            ...
            throw new RuntimeException("SM2 signing failed: " + e.getMessage(), e);
        }
    }
     ...
     * 验证内容的 SM2 签名。
     *
     * @param contentBytes   待验签内容（非 {@code null}）
     * @param signatureBytes 签名（DER 编码，非 {@code null}）
     * @return {@code true} 表示签名有效；签名无效或签名格式非法时返回 {@code false}
     */
    public boolean verify(byte[] contentBytes, byte[] signatureBytes) {
        Assert.notNull(contentBytes, "content bytes must not be null");
        Assert.notNull(signatureBytes, "signature bytes must not be null");
        try {
            var signer = new SM2Signer();
    ...
    private byte[] decodeText(String text) {
        return isHex(text) ? HexUtils.decodeToBytes(text) : Base64.getDecoder().decode(text);
    }
    ...
    private ECPrivateKeyParameters decodePrivateKey(byte[] bytes) {
        return new ECPrivateKeyParameters(new BigInteger(1, bytes), DOMAIN_PARAMS);
    }

    private ECPublicKeyParameters decodePublicKey(byte[] bytes) {
        try {
