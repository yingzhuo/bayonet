 ...
 * <p>默认按以下顺序查找 {@code default.properties}，后加载的覆盖先加载的（文件系统属性覆盖 classpath 属性）：</p>
 * <ol>
 ...
 */
@Slf4j
public class IncludePropertiesInitializer implements ApplicationContextInitializer<ConfigurableApplicationContext> {
    ...
    public void initialize(ConfigurableApplicationContext applicationContext) {
        for (String location : DEFAULT_LOCATIONS) {
            var resource = this.getResource(applicationContext, location);
            if (resource == null) {
                continue;
            }
            ...
            try (var inputStream = resource.getInputStream()) {
                ...
                var mapPropertySource = new MapPropertySource(location, map);
                applicationContext.getEnvironment()
                        .getPropertySources()
                        .addLast(mapPropertySource);
